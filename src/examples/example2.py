...
def Init():
    ...
    return (image_picker, text_picker, textbox, txtView, fileDown, singleList, multiList, imageOne, imageMulti, imageAndText, imageView, docView, docText)


@ftr.step(title="My entry step", description="my description")
def PreProcess(image_picker, text_picker, textbox, 
    txt, fileDown, singleList, multiList, imageOne, 
    imageMulti, imageAndText, imageView, docView, docText):

    f = open("src/examples/data/file.txt", "r")
    fileData = f.read()
    f.close()

    txt = ftr.Text.View(output_text=fileData)
    ...
    random_text = "Hello from my class"
    return image_picker, text_picker, textbox, txt, fileDown, singleList, multiList, imageOne, imageMulti, imageAndText, imageView, docView, docText, random_text
...
def SuperEvaluator(image_picker, text_picker, textbox, txt, fileDown, singleList, multiList, imageOne, imageMulti, imageAndText, imageView, docView, docText, random_text):
    print("Random text:", random_text, type(random_text))
    return [image_picker,
            text_picker,
            textbox,
            txt,
...
if __name__ == "__main__":
    ...
    ftr.build(name="Example2",
              init=Init, steps=[PreProcess, SuperEvaluator],
              file_bundle=bundle)
