...
class TrainVQG(pl.LightningModule):
    ...
    def calculate_losses(self, loss, kld, r=0.5):
        ...
        if kld is None:
            ...
            # self.kliter += 1
            # if self.args.variant == "icod-icod-l,lg,lv,ckl":
            ...
            # if self.args.variant in ("icod-icod-l,lg,lv,akl", "icodf-icodf-l,lg,lv,akl"):
    ...
    def validation_epoch_end(self, batch):
        ...
        qa_decode_scores = self.decode_and_print(batch)
...
#
    ...
    def test_step(self, batch, batch_idx):
        scores = self.decode_and_print(batch, val=False, random_testing=self.random_testing)
        for k, v in scores.items():
            rounded_val = np.round(np.mean(v) * 100, 4)
            print(k, "\t", rounded_val)
    ...
    def decode_and_print(self, batch, print_lim=20, val=True, random_testing=False):
        ...
        inference_ids = batch["input_ids"]
        inference_attention_masks = batch["input_attention_masks"]
        ...
        if random_testing:
            inference_ids = batch["random_inputs"]
            inference_attention_masks = batch["random_inputs_attn_masks"]
        ...
        for i, sentence in enumerate(decoded_sentences):
            ...
            if not val:
                if not image_id in self.outputs_to_save.keys():
                    self.outputs_to_save[image_id] = []

                value_obj = {
                    "category": curr_input.split()[0],
                    "objects": " ".join(curr_input.split()[1:]),
...
latent_early_stop_callback = EarlyStopping(
    monitor='val_qa_decode_msj_5',
    min_delta=0.00,
    patience=10,
    verbose=True,
    mode='max'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_latent_training_steps", type=float, default=35000,
    ...
    parser.add_argument("--latent_full_pretrain", type=lambda x: bool(strtobool(x)), default=False,
                        help="Whether to train a latent model after a full model training.")
    ...
    # if args.latent_full_pretrain:
    #     # trainVQG = TrainVQG.load_from_checkpoint("lightning_logs/version_175/checkpoints/epoch=5-v0.ckpt").to(device)
    #     trainVQG.turn_on_latent()
    #     trainer = pl.Trainer(max_steps=args.total_latent_training_steps, gradient_clip_val=5,
    #                          val_check_interval=50, limit_val_batches=350, callbacks=[latent_early_stop_callback], gpus=1)
    #     trainer.fit(trainVQG, data_loader, val_data_loader)
