...
class Latent(nn.Module):
    def __init__(self, args, rank3=False, dropout=0):
        ...
        self.mean_logvar_prior.apply(self.weights_init)
        self.mean_logvar_posterior.apply(self.weights_init)
    ...
    def weights_init(self, m):
        if isinstance(m, nn.Linear):
            if m.bias is not None:
                with torch.no_grad():
                    stdv = 1. / math.sqrt(5)
                    uniform = nn.parameter.Parameter(torch.FloatTensor(m.bias.shape).uniform_(0, stdv*2))
                    m.bias = uniform
    ...
    def forward(self, x, x_p):
        ...
        std = torch.exp(1 * logvar_prior) * 1.5  # used to be 0.5
